...
C
C WARNING: with respect to the original implementation of GMRES, the
C          matvec interface has been changed from matvec(x,y,n) to
C          matvec(n,x,y).
C
C  blas subroutines:
C   dscal
...
C     Calculate (initial) residual norm
      call matvec(n, x,work(1,workres))
      alpha = -1
c
...
C           u(1,k) := resid  
            ...
            call matvec(n,work(1,u+mod(k,j)),work(1,c+mod(k,j)))
            nits=nits+1
         else
...
c        initialize 1-st term  of rhs of hessenberg system..
 ...
 4       continue
            ...
            its = its + 1
            i1 = i + 1
            call  matvec(n, work0(1,i), work0(1,i1))
c           -----------------------------------------
