...
subroutine check_ddinit_args()
    ...
    call ddinit(n, x, y, z, rvdw, model, lmax, ngrid, force, fmm, pm, &
        & pl, iprint, nngmax, se, eta, eps, dd_data, info)
    if (info .ne. 0) stop 1
    call ddfree(dd_data)
    ...
    do i = -2, 2
        call ddinit(n, x, y, z, rvdw, model, lmax, ngrid, force, j, pm, &
            & i, iprint, nngmax, se, eta, eps, dd_data, info)
        if (info .ne. 0) stop 1
        call ddfree(dd_data)
    end do
    ...
    do i = 0, 10
        call ddinit(n, x, y, z, rvdw, model, lmax, ngrid, force, fmm, pm, &
            & pl, i, nngmax, se, eta, eps, dd_data, info)
        if (info .ne. 0) stop 1
        call ddfree(dd_data)
    end do
    ...
    do i = 1, 10
        call ddinit(n, x, y, z, rvdw, model, lmax, ngrid, force, fmm, pm, &
            & pl, iprint, i, se, eta, eps, dd_data, info)
        if (info .ne. 0) stop 1
        call ddfree(dd_data)
    end do
    ...
    tmp = -one
    call ddinit(n, x, y, z, rvdw, model, lmax, ngrid, force, fmm, pm, &
        & pl, iprint, nngmax, tmp, eta, eps, dd_data, info)
    if (info .ne. 0) stop 1
    call ddfree(dd_data)
    tmp = zero
    call ddinit(n, x, y, z, rvdw, model, lmax, ngrid, force, fmm, pm, &
    ...
    tmp = zero
    call ddinit(n, x, y, z, rvdw, model, lmax, ngrid, force, fmm, pm, &
        & pl, iprint, nngmax, se, tmp, eps, dd_data, info)
    if (info .ne. 0) stop 1
    call ddfree(dd_data)
    tmp = pt5
    call ddinit(n, x, y, z, rvdw, model, lmax, ngrid, force, fmm, pm, &
    ...
    tmp = zero
    call ddinit(n, x, y, z, rvdw, model, lmax, ngrid, force, fmm, pm, &
        & pl, iprint, nngmax, se, eta, tmp, dd_data, info)
    if (info .ne. 0) stop 1
    call ddfree(dd_data)
    tmp = pt5
    call ddinit(n, x, y, z, rvdw, model, lmax, ngrid, force, fmm, pm, &
