...
real(kind=rp),  parameter :: epsp = 1.0d0
!!
!! Taken from Chaoyu's MATLAB code
!!
!! wij          : (?) Maybe Eq. (54) 
real(kind=rp), allocatable :: wij(:,:)
real(kind=rp), allocatable :: coefvec(:,:,:), Pchi(:,:,:), &
                              & Qmat(:,:,:), coefY(:,:,:)
...
real(kind=rp), allocatable :: SI_ri(:,:), DI_ri(:,:), SK_ri(:,:), &
                              & DK_ri(:,:), termimat(:,:)
real(kind=rp)              :: tol_gmres, n_iter_gmres
...
contains
  ...
  real(kind=rp), intent(inout)    :: esolv
  real(kind=rp)                   :: inc, old_esolv
  real(kind=rp), intent(inout)    :: sigma(dd_data % nbasis,dd_data % nsph)
  real(kind=rp), intent(in)       :: phi(dd_data % ncav), &
                                        & gradphi(3,dd_data % ncav)
  real(kind=rp), intent(in)       :: psi(dd_data % nbasis, dd_data % nsph)
  real(kind=rp), intent(in)       :: charge(dd_data % nsph)
  !!
  ...
  !!
  real(kind=rp), allocatable ::   Xr(:,:), Xe(:,:), rhs_r(:,:), rhs_e(:,:), &
  ...
  real(kind=rp), allocatable :: g(:,:), f(:,:), g0(:), f0(:)
  integer                    :: isph
  integer                    :: i
  logical                    :: ok = .false.
  real(kind=rp)              :: tol
  ...
  implicit none
  type(dd_data_type), intent(in)  :: dd_data
  real(kind=rp), dimension(3, dd_data % ncav),       intent(in)  :: gradphi
  real(kind=rp), dimension(dd_data % ngrid, dd_data % nsph),    intent(out) :: f

  integer :: isph, ig, ic, ind, ind0, jg, l, m, jsph
  real(kind=rp) :: nderphi, sumSijn, rijn, coef_Ylm, sumSijn_pre, termi, &
      & termk, term
  real(kind=rp), dimension(3) :: sijn, vij
  real(kind=rp) :: rho, ctheta, stheta, cphi, sphi
  real(kind=rp), allocatable :: SK_rijn(:), DK_rijn(:)

  integer :: l0, m0, NM, kep, istatus
  real(kind=rp), dimension(dd_data % nbasis, dd_data % nsph) :: c0
  real(kind=rp), dimension(0:dd_data % lmax, dd_data % nsph) :: coef_bessel
  real(kind=rp), allocatable :: vplm(:), basloc(:), vcos(:), vsin(:)
  ...
  real(kind=rp), dimension(dd_data % nbasis, dd_data % nsph), intent(inout) :: y
  ...
  real(kind=rp), intent(inout) :: vector(dd_data % nbasis, dd_data % nsph)
  real(kind=rp) :: fac
  ...
  implicit none
  type(dd_data_type), intent(in)  :: dd_data
  real(kind=rp), dimension(dd_data % nbasis, dd_data % nsph), intent(in) :: rhs
  real(kind=rp), dimension(dd_data % nbasis, dd_data % nsph), intent(inout) :: Xe
  integer :: isph, istatus, n_iter, info, c1, c2, cr
  real(kind=rp) :: tol, r_norm
  ...
  real(kind=rp), dimension(dd_data % nbasis), intent(inout) :: basloc
  real(kind=rp), dimension(dd_data % nbasis), intent(inout) :: vplm
  real(kind=rp), dimension(dd_data % lmax+1), intent(inout) :: vcos
  real(kind=rp), dimension(dd_data % lmax+1), intent(inout) :: vsin
  real(kind=rp), dimension(dd_data % nbasis) :: fac_cosmo, fac_hsp
  integer :: its, ij, jsph
  real(kind=rp) :: rho, ctheta, stheta, cphi, sphi
  real(kind=rp) :: vij(3), sij(3)
  real(kind=rp) :: vvij, tij, xij, oij
  ...
  type(dd_data_type), intent(in)  :: dd_data
  integer, intent(in) :: isph
  real(kind=rp), intent(in) :: rijn, ri
  real(kind=rp), dimension(dd_data % nbasis), intent(in) :: basloc
  real(kind=rp), dimension(dd_data % nbasis), intent(inout) :: fac_hsp
  real(kind=rp), dimension(0:dd_data % lmax) :: SI_rijn, DI_rijn
  integer :: l, m, ind, NM
  ...
  implicit none
  type(dd_data_type), intent(in)  :: dd_data
  real(kind=rp),  intent(in) :: tij
  real(kind=rp), dimension(dd_data % nbasis), intent(in) :: basloc
  real(kind=rp), dimension(dd_data % nbasis), intent(inout) :: fac_cosmo
  integer :: l, m, ind
  do l = 0, dd_data % lmax
    do  m = -l, l
        ind = l*l + l + 1 + m
  ...
  implicit none
  type(dd_data_type), intent(in)  :: dd_data
  real(kind=rp), dimension(dd_data % nbasis,dd_data % nsph), intent(in) :: rhs_cosmo_init, &
      & rhs_hsp_init
  real(kind=rp), dimension(dd_data % nbasis,dd_data % nsph), intent(inout) :: rhs_cosmo, rhs_hsp
  real(kind=rp), dimension(dd_data % nbasis,dd_data % nsph) :: rhs_plus
  real(kind=rp), dimension(dd_data % nbasis,dd_data % nsph), intent(in) :: Xr, Xe
  integer :: isph, jsph, ig, kep, ind, l1,m1, ind1, ind0, count, istatus
  real(kind=rp), dimension(3) :: vij
  real(kind=rp), dimension(dd_data % nbasis,dd_data % nsph) :: diff_re
  real(kind=rp), dimension(nbasis0,dd_data % nsph) :: diff0
  real(kind=rp), dimension(dd_data % nbasis,dd_data % nbasis,dd_data % nsph) :: smat
  real(kind=rp), dimension(dd_data % ncav) :: diff_ep
  real(kind=rp) :: Qval, rijn, val
 ...
 999  format('Time of initializing Pchi, coefvec, termi: ',f8.3, &
  ...
  type(dd_data_type), intent(in)  :: dd_data
  integer,  intent(in) :: isph
  real(kind=rp), dimension(dd_data % nbasis, (lmax0+1)**2), intent(inout) :: pmat
